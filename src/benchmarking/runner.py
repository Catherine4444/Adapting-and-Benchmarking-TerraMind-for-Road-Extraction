...
MODEL_FAMILIES = ("unet", "sr", "terratorch")
...
def load_predictor(model: str, checkpoint, device: str, sen2sr_dir=None, tm_data_config = None):
    if model == "unet":
    ...
    if model == "terratorch":
        import yaml
        dm = yaml.safe_load(open(tm_data_config))["init_args"]
        return TerraTorchPredictor(
            checkpoint, device,
            bands=dm["bands"], norm_mean=dm["norm_mean"], norm_std=dm["norm_std"],
            modality_output=dm.get("modality_output", "split"))     # split = TerraMind decoder
...
class TerraTorchPredictor:
    family = "terratorch"
    scale = 1

    def __init__(self, checkpoint, device, *, bands, norm_mean, norm_std,
                 modality_output="split", num_classes=2, positive_class=1):
        import terratorch                       # noqa: registers factories
        TerraTorchPredictor._import_custom_module()
        from terratorch.tasks import SemanticSegmentationTask
        # hparams in the .ckpt already carry model_factory + model_args
        # (incl. peft_config for LoRA), so a plain load reconstructs the
        # LoRA-wrapped architecture and matches the state_dict:
        self.model = SemanticSegmentationTask.load_from_checkpoint(
            checkpoint, map_location="cpu").eval().to(device)
        self.device = device
        self.bands = list(bands)               # [1..12], same as base_data.yml
        self.norm_mean, self.norm_std = norm_mean, norm_std
        self.modality_output = modality_output # "split" (terramind) | "stacked" (baselines)
        self.num_classes, self.positive_class = num_classes, positive_class
            # in __init__, add:
        self.threshold = 0.5
        self.img_size = 256          # backbone_img_size; or read from model_args

    def read_chip(self, src, window):
        from custom_modules.norm import read_window, apply_norm
        img = read_window(src, self.bands, window)          # (C,h,w)
        return apply_norm(img, self.bands, self.norm_mean, self.norm_std)

    @torch.no_grad()
    def predict_probs(self, imgs):
        from custom_modules.norm import split_modalities
        h, w = imgs[0].shape[-2:]
        x = np.stack([np.ascontiguousarray(i) for i in imgs])   # (B,C,h,w)
        if self.modality_output == "split":
            # terramind wants {"S2L2A":..., "S1GRD":...}, not a stacked tensor
            image = TerraTorchPredictor.split_modalities_batched(x, self.bands)     # dict of (B,Ck,h,w) tensors
        else:
            image = torch.from_numpy(x).float()
        t0 = time.perf_counter()
        out = self.model(TerraTorchPredictor._to_device(image, self.device))
        logits = out.output if hasattr(out, "output") else out  # ModelOutput
        ms = (time.perf_counter() - t0) * 1000.0
        prob = torch.softmax(logits, dim=1)[:, self.positive_class:self.positive_class+1]
        return prob[..., :h, :w].cpu(), ms
    
    def split_modalities_batched(x, bands):
        """(B,C,H,W) ndarray -> {"S2L2A": (B,Ck,H,W), "S1GRD": (B,Ck,H,W)} tensors,
        using the exact per-sample split_modalities from training."""
        from custom_modules.norm import split_modalities
        dicts = [split_modalities(x[b], bands) for b in range(x.shape[0])]
        return {k: torch.stack([d[k] for d in dicts]) for k in dicts[0]}

    def _to_device(image, device):
        if isinstance(image, dict):
            return {k: v.to(device) for k, v in image.items()}
        return image.to(device)

    def _import_custom_module():
        import sys
        from pathlib import Path
        _FT = Path(__file__).resolve().parents[1] / "finetuning_tm"   # src/finetuning_tm
        if str(_FT) not in sys.path:
...
def evaluate(dataset_dir, checkpoint, model_name, seed, store_dir, split="test",
             ...
             stratum=None, stratum_col=None, buffer_px=None, ap_bins=None, tm_data_config = None):
    ...
    pred = load_predictor(model, checkpoint, device, sen2sr_dir=sen2sr_dir, tm_data_config=tm_data_config)
